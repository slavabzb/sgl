    ...
    void test_convert()
    {
        for(std::size_t i = 0; i < this->array_size; ++i)
        {
            ...
            TS_ASSERT_THROWS_NOTHING(
                char_type = sgl::view::converter::convert(this->types[i])
            );
            ...
            TS_ASSERT_THROWS_NOTHING(
                type = sgl::view::converter::convert(this->char_types[i])
            );
        ...
        TS_ASSERT_THROWS(
            sgl::view::converter::convert('X'),
            std::out_of_range
        );
        
        TS_ASSERT_THROWS(
            sgl::view::converter::convert(static_cast<sgl::view::type>(100)),
            std::out_of_range
        );
    }
};
